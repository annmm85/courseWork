...
//    создание пользователя - регистрация уже создана
    ...
    public
    function readAuthors(Request $request): JsonResponse
    {
    ...
    public
    function readSubscribers(Request $request): JsonResponse
    {
    ...
    public
    function unsubscribe(Request $request, $id): JsonResponse
    {
        $authors = User::find($request->user()->id)->authors();
        if (User::find($id)) {
            if (in_array($id, $authors->pluck('id')->toArray())) {
